...
for line in open(fasta_sequence_file, 'r'):
    if line[0] == ">":
        ...
        string_index += 1
        sequence_meta_list.append(line[1:9].strip())
        continue
    else:
        line = line.strip()
...
#print(df)
format_row = "{:>10}" * (len(sequence_meta_list) + 1)
print(format_row.format("", *sequence_meta_list))
for team, row in zip(sequence_meta_list, matrix):
